...
if [ -f ~/.cache/hyprland-wallpapers/wallpaper_cache_enabled ]; then
    use_cache=1
...
blurfile="$HOME/.cache/hyprland-wallpapers/blur_enabled"
defaultwallpaper="$HOME/.config/hypr/wallpapers/default.jpg"
wallpapereffect="$HOME/.cache/hyprland-wallpapers/wallpaper_effect"
blur="50x30"
blur=$(cat $blurfile)
...
if [ -f $wallpapereffect ]; then
    ...
    if [ ! "$effect" == "off" ]; then
        ...
        set_hyprpaper_wallpaper "$used_wallpaper"
    else
...
# -----------------------------------------------------
# Reload nwg-dock-hyprland (if running)
# -----------------------------------------------------

if pgrep -x "nwg-dock-hyprla" > /dev/null; then
    pkill -x "nwg-dock-hyprla" && sleep 0.2 && nwg-dock-hyprland &
fi
