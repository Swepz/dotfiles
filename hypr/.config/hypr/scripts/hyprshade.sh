...
if [[ "$1" == "rofi" ]]; then
    ...
    if [ ! -z $choice ]; then
        echo "hyprshade_filter=\"$choice\"" >~/.config/hypr/settings/hyprshade
        if [ "$choice" == "off" ]; then
            hyprshade off
    ...
    # Check if hyprshade settings file exists and load
    if [ -f ~/.config/hypr/settings/hyprshade ]; then
        source ~/.config/hypr/settings/hyprshade
    fi
